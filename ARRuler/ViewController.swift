...
class ViewController: UIViewController, ARSCNViewDelegate {
    ...
    func calculate() {
        ...
        let distance = sqrt(pow(endPosition.position.x - startPosition.position.x, 2) +
                            pow(endPosition.position.y - startPosition.position.y, 2) +
                            pow(endPosition.position.z - startPosition.position.z, 2))
        
        updateText(text: "\(abs(distance * 100))", atPosition: endPosition.position)
    }
